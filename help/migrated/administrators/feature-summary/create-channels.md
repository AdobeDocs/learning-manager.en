...
1. Login to Adobe Learning Manager as an Administrator.

2. Select **Channels** from the left navigation.
<br> The **Channels** page opens.

3. Select the **Settings** tab.
![Enable Channels Feature](assets/enable-channels-feature.png) Enable the Channel feature in the **Settings** tab to let Administrators create channels for the account.
...
![Create Channel Content Source](assets/create-channel-content-source.png) Define the content source and configure visibility and sync options when creating a channel.

2. In the **Channel** section, enter the **Channel name** and **Description**.

3. Select a **Source type** from the drop-down menu. The following options are available:

   1. **Web page**: Select this option to crawl a web page and import video links along with their associated metadata.

   2. **Confluence page**: Select this option to retrieve video links and metadata from a Confluence Cloud page. To connect to Confluence Cloud, provide the following details:
    ...
    ![Cloud Confluence page](assets/cloud-confluence-page.png) Enter the Source URL, along with the Atlassian email address and API token used to authenticate with Confluence Cloud.

4. Enter the **Source URL** of the selected source type content.

5. In the **Status** section, configure the following options:
   ...
   2. **Sync automatically**: Enable this option to automatically update the channel when new videos are added to the source. Disable it if you want to manually sync the channel.

6. (Optional) Select **Show advanced settings**, and then configure the following options as required:

   1. **Channel theme color**: Select a color to customize the visual appearance of the channel.

   2. **Crawl depth**: Enter the crawl depth for linked pages to scan for video content. It supports a maximum crawl depth of **2**.

   3. **Crawl frequency (in hours)**: Enter how often Adobe Learning Manager should check the source for new or updated content.
![Channel Content Check Schedule](assets/channel-content-check-schedule.png) Select Show advanced settings to configure the channel theme color, crawl depth, and crawl frequency

1. Select **Test now** to validate the source.
The sample videos are retrieved and displayed from the configured source.
![Test Channel Source Connection](assets/test-channel-source-connection.png) Use **Test now** to confirm videos are retrieved from the source before you create the channel.
...
1. Select the required channel from the **Channels** list.<br>
The **Edit channel** page opens and displays the current channel configuration.

2. Update the channel settings as needed.
![Edit Channel Settings](assets/edit-channel-settings.png) Update a channel's name, description, source, and settings from the **Edit channel** page.

3. (Optional) Select **Test now**.

4. Select **Save changes**.<br>
