...
## Multiple custom role

Administrators can assign multiple roles to a single user with specific permissions across users, catalogues, and learning features. Roles can be assigned through the **[!UICONTROL Users]** page or via **CSV import** for bulk updates.

When a custom admin leaves the organization, roles can be reassigned to another existing custom admin. Users can easily switch between their assigned roles using the profile dropdown.

>[!NOTE]
>
>A maximum of **50 roles per user** and **500 users per role** is allowed.  

If a user has more than 50 roles assigned through the UI, you can replace old roles using the UI. Once roles are assigned via CSV, they **cannot be modified** from the UI.

### Assign Custom Roles to Users

To assign custom roles to a user, follow these steps:

1. Log in as an administrator.
2. Go to **[!UICONTROL Users]** > **[!UICONTROL Custom Roles]** and select **[!UICONTROL Create Custom Role]**.

   ![](assets/create-custom-role-1.png)
   _Create custom role_

3. Type the custom role name and description.
4. Select the required privileges from the following:
...
5. Search for the users and select the required users under the **[!UICONTROL User]** option.
6. Select **[!UICONTROL Save]**.

Users receive email and in-app notifications when roles are assigned. Notifications include the name of the assigned role. The email notification is handled using the **Role Assigned-Custom** email template.

To assign another role to the same user:

1. Create a new custom role.
2. Add the same users to the new role.

>[!NOTE]
>
>After you assign roles, you can see how many more roles the user can still have.

![](assets/custom-role-limit.png)
_Custom role limit_

### Add or Remove Custom Roles

Once you assign custom roles to users, you can modify, add, or remove those roles.

1. Search for the user on the **[!UICONTROL Users]** page.
2. From the user dropdown, select **[!UICONTROL Manage Custom Roles]**.
   
   ![](assets/manage-custom-role.png)
   _Manage custom roles_

3. In the dialog, select **[!UICONTROL Add/Remove Roles]**.
   
   ![](assets/add-or-remove-custom-role.png)
   _Add or remove custom role_

4. Search for and assign new roles or remove existing ones.
5. Select **[!UICONTROL Save]** and confirm the role change when prompted.

>[!NOTE]
>
> You can add a third or more roles like this, as long as the limit is not exceeded.

### Importing Roles via CSV

The **CSV import workflow** allows bulk role assignments. Prepare two CSV files:

* `role.csv` for role definitions  
* `user_role.csv` for role assignments

Upload both the CSVs via the **Box workflow**.

>[!NOTE]
>
>If the CSV file contains invalid data or exceeds specified limits, a **notification** will display the errors.

Error Notification for Custom Roles

The system displays an error message when CSV imports contain invalid data or exceed the allowed limits.
